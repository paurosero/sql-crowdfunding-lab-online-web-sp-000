...
"SELECT users.name, SUM(pledges.amount) FROM users JOIN pledges ON users.id = pledges.user_id GROUP BY user.name ORDER BY SUM(pledges.amout);"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM pledges JOIN pledges ON pledges.id = pledges.project_id WHERE projects.category = 'books';"
end
